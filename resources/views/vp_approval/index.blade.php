        ...
        @else
            <div class="grid gap-4 md:grid-cols-2">
                @foreach ($progres as $p)
                    <div class="bg-white rounded-md shadow p-4 w-full flex flex-col justify-between border">
                        ...
                        <div class="mt-4 space-y-3">
                            {{-- Form Approve --}}
                            <form action="{{ route('vp.approval.approve', $p->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                            ...
                            {{-- Form Reject --}}
                            <form action="{{ route('vp.approval.reject', $p->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
